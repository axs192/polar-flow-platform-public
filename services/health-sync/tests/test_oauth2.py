...
class TestGetAccessToken(unittest.TestCase):
    ...
    def test_success_returns_parsed_json_and_posts_expected_form_body(self):
        ...
        result = _client().get_access_token(authorization_code="auth-code")

        self.assertEqual(result, {"access_token": "abc123"})
        self.assertIn("code=auth-code", responses.calls[0].request.body)
...
class TestRequestDispatch(unittest.TestCase):
    ...
    def test_get_with_access_token_uses_bearer_auth_and_returns_json(self):
        responses.add(responses.GET, f"{BASE_URL}/exercises", json={"exercises": []}, status=200)
    ...
    @responses.activate
    def test_404_raises_http_error_with_status_in_message(self):
        responses.add(responses.GET, f"{BASE_URL}/exercises", json={"error": "not_found"}, status=404)
