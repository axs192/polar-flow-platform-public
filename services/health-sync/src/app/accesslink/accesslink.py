...
from src.app.accesslink import endpoints
...
class AccessLink:
    ...
    def __init__(self, client_id, client_secret, redirect_url=None):
        ...
        self.oauth = OAuth2Client(
            ...
            access_token_url=OAUTH_EXCHANGE_URL,
        ...
        self.activity = endpoints.Activty(oauth=self.oauth)
        self.heart_rate = endpoints.Heart_Rate(oauth=self.oauth)
        self.sleep = endpoints.Sleep(oauth=self.oauth)
        self.recharge = endpoints.Recharge(oauth=self.oauth)
        self.exercises = endpoints.Exercises(oauth=self.oauth)
